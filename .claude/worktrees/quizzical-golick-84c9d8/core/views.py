...
def school_add(request):
    ...
    if request.method == 'POST':
        ...
        if not name or not password:
            messages.error(request, 'School name and password are required.')
        else:
...
def school_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if new_logo:
            ...
            school.logo = None
        # Subdomain update — check uniqueness before saving
...
def super_school_fee_dashboard(request):
    ...
    for school in schools:
        ...
        monthly_bill = school.fee_per_student * active_students
        ...
        for session in all_sessions:
            ...
            balance = max(monthly_bill - paid, Decimal('0.00'))
...
def super_collect_fee(request, school_pk):
    ...
    paid_months = set()
    for bp in SchoolBillingPayment.objects.filter(school=school, academic_session=current_session).values_list('payment_months', flat=True):
        for m in (bp or []):
            paid_months.add(str(m))
    ...
    if request.method == 'POST':
        ...
        # Validate selected months belong to this session's billing window
        valid_tokens = {val for val, _ in all_billing_months}
        selected_months = [m for m in selected_months if m in valid_tokens and m not in paid_months]
    ...
    billing_history = SchoolBillingPayment.objects.filter(school=school, academic_session=current_session).order_by('-payment_date')
